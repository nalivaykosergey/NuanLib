...
#include <cmath>
#include <iomanip>
#include "../lib/integration/euler.hpp"

using std::cout, std::endl, std::setw, std::vector;
using nuan::integration::euler_method;
...
int main()
{
    ...
    vector<long double> solution = euler_method(f, initial_value, a, b, step);
    N = solution.size();
    x = a;

    cout << "Numerical solution of the Cauchy"
            "problem by the Euler method:" << endl;
    cout.precision(20);
    cout <<
             setw(30)  << "| x |" <<
             setw(30) << "| approx_y |" <<
