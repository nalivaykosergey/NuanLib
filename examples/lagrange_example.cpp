...
#include <cmath>
#include <iomanip>
#include "../lib/interpolation/lagrange.hpp"

using std::cout, std::endl, std::setw;
using nuan::interpolation::lagrange_polynomial;
...
int main() {
    ...
    auto *lagrange_values = new double[M + 1];
    ...
    for(size_t i = 0; i<=M; i++) {
        lagrange_values[i] = lagrange_polynomial(x1, y1, N + 1, x2[i]);
        deltaValues[i] = delta_value(lagrange_values[i], y2[i]);
        cout
                << setw(10)   << x2[i]
                << setw( 25 ) << y2[i]
                << setw( 25 ) << lagrange_values[i]
