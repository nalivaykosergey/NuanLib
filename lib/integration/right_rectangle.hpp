#ifndef NUMERICAL_ANALYSIS_LIB_RRECTANGLE_INTEGRATION_HPP
#define NUMERICAL_ANALYSIS_LIB_RRECTANGLE_INTEGRATION_HPP
...
namespace nuan {
    ...
    {
        ///  @brief Numerical solution of the integrals using right rectangles
        ...
        template<typename T>
        T right_rectangle(T (*func)(T), T a, T b, T step = 0.01)
        {
            if(a >= b)
                throw errors::IntegrationRangeError();
            T approximated_I = 0.0,  x = a + step;
            while(x <= b)
            {
        ...
        /// \example right_rectangle_example.cpp
        /// This is an example of how to use the right_rectangle.
